...
export function PickerOverlay({ children, selectedItem }: PropsWithChildren<PickerOverlayProps>) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-slate-950/70 p-4">
      <button aria-label={t("close")} className="absolute inset-0" onClick={() => cancelPick()} type="button" />
      <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl bg-slate-50 shadow-2xl">
        {children}
